...
from .models import (Category, Algorithm, User,
                     ImageAlgorithm, UrlAlgorithm,
                     Comment, TaskAlgorithm, UrlTaskAlgorithm)
...
class CategoryDetailView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['urls'] = get_object_or_404(
            Category, slug=self.kwargs['category']).urls.all()
        context['images'] = get_object_or_404(
            Category, slug=self.kwargs['category']).images.all()
        return context
...
class AlgorithmDetailView(DetailView):
    ...
    def dispatch(self, request, *args, **kwargs): 
        self.algorithm = get_object_or_404(
            Algorithm, slug=kwargs['pk_algorithm'])
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['urls'] = UrlAlgorithm.objects.filter(
            algorithm=self.algorithm.pk)
        context['images'] = ImageAlgorithm.objects.filter(
            algorithm=self.algorithm.pk)
        context['tasks'] = TaskAlgorithm.objects.filter(
            algorithm=self.algorithm.pk)
        context['online_tasks'] = UrlTaskAlgorithm.objects.filter(
            algorithm=self.algorithm.pk)
...
class CommentFormMixin:
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if instance.author != request.user:
            return redirect(
                'algorithm:algorithm_detail',
                self.kwargs['pk_category'], self.kwargs['pk_algoritm']
            )
        return super().dispatch(request, *args, **kwargs)

    def get_success_url(self):
        return reverse('algorithm:algorithm_detail',
                       kwargs={'pk_category': self.kwargs['pk_category'],
                               'pk_algorithm': self.kwargs['pk_algorithm']}
        )
...
class CommentCreateView(LoginRequiredMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        self._algorithm = Algorithm.objects.filter(
            category__slug=self.kwargs['pk_category']).get(slug=kwargs['pk_algorithm']
        ) 
        return super().dispatch(request, *args, **kwargs)
    ...
    def get_success_url(self):
        return reverse(
            'algorithm:algorithm_detail',
            kwargs={'pk_algorithm': self._algorithm.slug,
                    'pk_category': self._algorithm.category.slug}
        )
...
class TheoryAlgorithms(TemplateView):
    template_name = 'algorithm/theory_algorithm.html'


class AlgorithmInStructure(TemplateView):
    template_name = 'algorithm/algorithm_in_structure.html'


class OptimizeAlgoritm(TemplateView):
    template_name = 'algorithm/optimize_algoritm.html'
